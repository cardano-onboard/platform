    ...
    Vue.mixin({
      ...
      methods: {
        ...
        findInputs(_utxo, need) {
          console.log(_utxo, _utxo.len(), need);
          const segregated = {
            lovelaceOnly: [],
            withTokens: [],
          };
          ...
          segregated.lovelaceOnly.sort((a, b) => {
            const a_lovelace = BigInt(a.output().amount().coin().to_str());
            const b_lovelace = BigInt(b.output().amount().coin().to_str());
          ...
          const inputs = [];
          let input_lovelace = 0n;
          // let input_tokens = {};
          if (need.lovelace && (need.assets === null || need.assets === undefined)) {
            // We only need lovelace!
              const needed_lovelace = BigInt(need.lovelace) + 1000000n;

            while (segregated.lovelaceOnly.length) {
              console.log(input_lovelace, needed_lovelace);
              ...
              const lovelace = BigInt(to_use.output().amount().coin().to_str());
              inputs.push(to_use);
