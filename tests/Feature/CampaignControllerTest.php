...
class CampaignControllerTest extends TestCase
{
    ...
    public function test_campaign_show_includes_performance_stats(): void
    {
        Http::fake(['*' => Http::response([
            'status' => 'ok',
            'data' => [null, ['liveUtxos' => []]],
        ...
        // One code with capacity for 5 claims; record 2 claims against it.
        $code = Code::factory()->for($campaign)->create(['uses' => 5, 'lovelace' => 2000000]);
        Claim::factory()->count(2)->for($code)->create();
        ...
        $this->assertArrayHasKey('claims_over_time', $stats);
        $this->assertArrayHasKey('claimed_vs_unclaimed', $stats);
        $this->assertArrayHasKey('code_utilization', $stats);

        // 2 claims recorded against a 5-use code => 2 claimed, 3 unclaimed reward slots.
        $this->assertSame(2, $stats['claimed_vs_unclaimed']['claimed']);
        $this->assertSame(3, $stats['claimed_vs_unclaimed']['unclaimed']);

        // Utilization buckets: one code, claimed (has claims) and still available (2 < 5).
        $this->assertSame(1, $stats['code_utilization']['total']);
        $this->assertSame(1, $stats['code_utilization']['claimed']);
        $this->assertSame(1, $stats['code_utilization']['available']);
        $this->assertSame(0, $stats['code_utilization']['exhausted']);

        // Cumulative timeline reflects the 2 claims on a single day.
        $this->assertNotEmpty($stats['claims_over_time']);
        $last = end($stats['claims_over_time']);
        $this->assertSame(2, $last['cumulative']);
    }

    public function test_campaign_show_includes_lifecycle_status(): void
    {
        // The frontend gates ended-campaign actions (disables Add/Import Codes and Top Up)
        // on campaign.status, so show() must serialize the status accessor. It was once
        // omitted from the ->only() field list, leaving those controls live on ended campaigns.
        Http::fake(['*' => Http::response(['status' => 'ok', 'data' => [null, ['liveUtxos' => []]]])]);

        $user = User::factory()->create();
        $campaign = $this->createCampaignWithWallet($user);
        $campaign->update(['start_date' => '2026-01-01', 'end_date' => '2026-01-31']);

        $this->travelTo('2026-02-15');  // past the end date
        $response = $this->actingAs($user)->get(route('campaigns.show', $campaign));
        $this->travelBack();

        $props = $response->original->getData()['page']['props'];
        $this->assertSame('ended', $props['campaign']['status']);
    }

    public function test_campaign_show_includes_code_rewards(): void
    {
        Http::fake(['*' => Http::response([
            'status' => 'ok',
            'data' => [null, ['liveUtxos' => []]],
        ...
        $user = User::factory()->create();
        $campaign = $this->createCampaignWithWallet($user);
        $code = Code::factory()->for($campaign)->create(['uses' => 1, 'lovelace' => 2000000]);
        \App\Models\Reward::factory()->for($code)->create([
        ...
        $response->assertOk();
        $reward = $response->original->getData()['page']['props']['campaign']['codes'][0]['rewards'][0];

        $this->assertSame('484f534b59', $reward['asset_hex']);
        $this->assertSame(5, $reward['quantity']);
        // Sensitive reward columns stay hidden from serialization.
        $this->assertArrayNotHasKey('metadata', $reward);
        $this->assertArrayNotHasKey('policy_id', $reward);
    }
